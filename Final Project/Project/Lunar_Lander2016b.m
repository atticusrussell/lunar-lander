...
global dx_shp dy_shp quitgame theta tinc Vx Vy thrust mass fuel mass_f distmoon stagereset landings maxthrust xmax ymax xmin ymin missed landingphase resetcount surfaceslopedeg
...
SHIPlegs=15; %the distance from the ground that the ship will crash/land
...
resetcount=0;
surfaceslopedeg=0;
fys=1; %temporary flame scale at one
landings=0;
%% create the figure
...
[SHIP,Shp_Patch,f]=shiprender(fys);
...
global quitgame theta thrust maxthrust
val = event.Key;
    switch val
    case 'space'
        thrust = maxthrust;
...
function [SHIP,Shp_Patch,f] = shiprender(fys)
...
function [flamematadj]= flame(fys)
fxscale=0.9;       
flamex=[-15,000,015,    020,    025,     030,     035,     025,     028,     020,     018,     015,     010,     005,     002,     000,     -02,     -05,     -10,     -15,     -18,     -20,     -28,     -25,     -35,     -30,    -25,    -20];
flamexadj=flamex*fxscale;
flamey=[-20,-20,-20,-50*fys,-70*fys,-100*fys,-150*fys,-140*fys,-180*fys,-150*fys,-170*fys,-180*fys,-200*fys,-160*fys,-205*fys,-225*fys,-205*fys,-160*fys,-200*fys,-180*fys,-170*fys,-150*fys,-180*fys,-140*fys,-150*fys,-100*fys,-70*fys,-50*fys];
...
global stagereset distmoon dy_shp dx_shp Vy Vx fuel xmax xmin ymax ymin theta surfaceslopedeg landings
    if dy_shp<1000
        %initialize counter
        n=1;
        %check for collisons
        ...
        if distmoon<SHIPlegs
            ...
            if -10<Vy && abs(Vx)<10 && 20>abs(theta-surfaceslopedeg)
                ...
                if land == true
                    ...
                    wintext=text((xmax-xmin)/2,(ymax-ymin)/2,'Successful Landing: +100 fuel','color','g','FontSize',49);
                ...
                if crash == true
                    ...
                    failtext=text(dy_shp,dx_shp,'You Crashed: -100 fuel','color','r','FontSize',15);
                    %pause the program for a second
                    pause(1)
                    %delete the text
                    delete(failtext)       
...
global stagereset fuel dx_shp dy_shp 
    if dx_shp<1 || dx_shp>1799 || dy_shp>999
        %display the fact that it crashed in text
        missedtext=text(300,500,'You Missed the Landing Zone: -100 fuel','color','r','FontSize',30);
        %pause the program for a second
        pause(1)
        %delete the text
        delete(missedtext)       
